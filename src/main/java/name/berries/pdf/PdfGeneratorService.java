 ...
 * 
 * This service provides methods to convert HTML content or URLs to PDF files
 * with configurable options for page format, margins, and rendering behavior.
 */
public class PdfGeneratorService {
    ...
    /**
     * Loads external stylesheets into the page.
    ...
    private void generatePdf(Page page, String outputPath, PdfOptions options) throws Exception {
        ...
        // Set format
        pdfOptions.setFormat(options.getFormat());
        ...
        Margin margin = new Margin()
                .setTop(options.getMarginTop())
                .setBottom(options.getMarginBottom())
                .setLeft(options.getMarginLeft())
                .setRight(options.getMarginRight());
        pdfOptions.setMargin(margin);
        
        // Set print background
        pdfOptions.setPrintBackground(options.isPrintBackground());
        ...
        if (options.isDisplayHeaderFooter()) {
            ...
            if (options.getHeaderTemplate() != null) {
                pdfOptions.setHeaderTemplate(options.getHeaderTemplate());
            }
            if (options.getFooterTemplate() != null) {
                pdfOptions.setFooterTemplate(options.getFooterTemplate());
            }
        }
        
        // Set output path
        pdfOptions.setPath(outputFilePath);
        
        logger.debug("PDF options: format={}, landscape={}, printBackground={}, margins={}|{}|{}|{}", 
                options.getFormat(), options.isLandscape(), options.isPrintBackground(),
